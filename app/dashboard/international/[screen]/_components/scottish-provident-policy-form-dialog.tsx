...
export function ScottishProvidentPolicyFormDialog({ policy }: Props) {
  const isEdit = !!policy;
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const result = isEdit
      ? await updateIntlPolicy(policy.id, "scottish-provident", fd)
      : await createIntlPolicy("scottish-provident", fd);
        ...
        <DialogContent className="sm:max-w-5xl max-h-[92vh] overflow-y-auto p-6">
          <DialogHeader>
            <DialogTitle className="text-xl">
              {isEdit ? "Edit Scottish Provident Policy" : "New Scottish Provident Policy"}
            </DialogTitle>
          </DialogHeader>
          ...
          <form onSubmit={handleSubmit} className="py-2">
            ...
            <div className="grid grid-cols-2 gap-x-10 gap-y-4">
              ...
              <div className="space-y-4">
                ...
                {renderInput("contribution_escalation", "Contribution Escalator Rate")}
                {renderInput("smoker_status", "Smoker / Non Smoker")}
                {renderInput("additional_protection", "Additional Protection Benefits")}
